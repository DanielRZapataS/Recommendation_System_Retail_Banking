...
crediservice_master <- list.files(crediservice_path)
crediservice <- fread(os.path.join(crediservice_path, crediservice_master))
crediservice[, periodo := as.Date(periodo)]
# target 
crediservice[, target := ifelse(pr_crediservice_2monthsFurther - pr_crediservice > 0, 1, 0)]
crediservice[, sum(target, na.rm = T), by = periodo]
crediservice[, pr_crediservice_2monthsFurther := NULL]
crediservice[is.na(crediservice)] <- 0
...
test1 <- crediservice[periodo == test_cut1]
test2 <- crediservice[periodo == test_cut2]

prediccion1 <- crediservice[periodo == score1]
prediccion2 <- crediservice[periodo == score2]


crediservice <- crediservice[periodo >= train_cut_min & 
                       periodo <= train_cut]
...
crm.vars <- names(crediservice)[!grepl("pr_*", names(crediservice))]
no.crm.vars <- c("month.id", "llave", "month", "year", 
                 "num.transactions", "target", "total_products")
crm.vars <- crm.vars[crm.vars %!in% no.crm.vars]
lagged.vars <- names(crediservice)[grepl("month_ago", names(crediservice))]
last.owned.vars <- names(crediservice)[grepl("*last.owned", names(crediservice))]
owned.within.vars <- names(crediservice)[grepl("owned.within", names(crediservice))]
purchase.count.vars <- names(crediservice)[grepl("purchase.count", names(crediservice))]

sapply(crediservice[, mget(crm.vars)], class)

categorical.cols <- c(crm.vars[sapply(crediservice[, mget(crm.vars)], is.factor)],
                      "month", "year")
...
products.interest <- c("nomina", "crediservice", "ahorros",
                       "libranza", "otros", "libredestino",
                       "crediservice", "cdt")

products.interest <- names(crediservice)[which(rowSums(
  sapply(products.interest, grepl,  names(crediservice))) == 1)]

categorical.cols <- categorical.cols[categorical.cols %in% products.interest]

numeric.cols <- c(crm.vars[sapply(crediservice[, mget(crm.vars)], is.numeric)], 
                  purchase.count.vars[purchase.count.vars %in% products.interest],
                  "total_products", "num.transactions")


# ## making sub-sub sample 
# cedulas <- unique(crediservice$llave)
# length(cedulas)
# n <- length(cedulas)/12 %>% round()
# mysample <- sample(cedulas, n, replace = FALSE )
# length(mysample)
# 
# crediserviceSample <- crediservice[llave %in% mysample]
# crediserviceSample


# one-hot encode the categorical features
ohe <- dummyVars( ~ ., data = crediservice[, mget(categorical.cols)])
ohe <-
  as(data.matrix(predict(ohe, crediservice[, mget(categorical.cols)])), "dgCMatrix")
...
target <- list()
target <- as(data.matrix(crediservice$target),'dgCMatrix')
...
save.id       <- crediservice$llave
save.month.id <- crediservice$month.id
save.month    <- crediservice$month
...
crediservice         <- cbind(ohe,data.matrix(crediservice[,mget(numeric.cols)]))
...
dtrain <- xgb.DMatrix(data = crediservice, label=target)
model <- xgboost(
  data = dtrain,
  nround = 100,
...
xgb.save(model, "Models/xgboost_crediservice.model")
model <-  xgb.load("Models/xgboost_crediservice.model")
...
fwrite(gaintable, paste0("Results/upliftTest_crediservice", test_months[1],".csv"))
fwrite(predictions, paste0("Results/predictionTest_crediservice",  test_months[1],".csv"))
...
fwrite(gaintable, paste0("Results/upliftTest_crediservice", test_months[2],".csv"))
fwrite(predictions, paste0("Results/predictionTest_crediservice",  test_months[2],".csv"))
########################### 
...
fwrite(predsmtrix, "Results/prediction_crediservice_201809.csv")

###########################

## Delecting variables ##

crediservice1 <- as.matrix(crediservice)
crediservice1 <- data.table(crediservice1)
...
crediservice1 <- crediservice1[, mget(selectedvars)]
test11 <- test11[, mget(selectedvars)]
test21 <- test21[, mget(selectedvars)]

crediservice1 <- as(data.matrix(crediservice1),'dgCMatrix')
test11 <- as(data.matrix(test11),'dgCMatrix')
test21 <- as(data.matrix(test21),'dgCMatrix')

dtrain1 <- xgb.DMatrix(data = crediservice1, label=target)
model1 <- xgboost(
  data = dtrain1,
  nthread = 4,
  nround = 100,
