...
create_model <- function(train_months,
  ...
  searchGridSubCol <- expand.grid(max_depth = c(4, 6, 8, 10, 12, 14),
  ...
  system.time(
    ErrorsHyperparameters <- apply(searchGridSubCol, 1, function(parameterList){
      ...
      output <- return(c(testaucpr, trainaucpr, current_max_depth, current_eta, current_gamma, current_alpha, current_early_stoping_round, current_nrounds))}))
  
  output <- as.data.frame(t(ErrorsHyperparameters))
  varnames <- c("Testaucpr", "Trainaucpr", "max_depth", "eta", "gamma", "alpha", "early_stoping", "nrounds")
